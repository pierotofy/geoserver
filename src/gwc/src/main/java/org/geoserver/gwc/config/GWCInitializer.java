...
/**
 * GeoSever initialization hook that preserves backwards compatible GWC
 * configuration at start up.
 * <p>
 * For instance, this initializer:
 * <ul>
 * <i> Creates a <data directory>/gwc-gs.xml configuration file if it doesn't
 * exist, populated with old defaults global configuration for gwc layers based
 * on geoserver layers and layer groups.
 * <li>Configures a gwc {@link GeoServerTileLayerInfoImpl tile layer} for every
 * {@link LayerInfo} and {@link LayerGroupInfo} matching the old defaults, also
 * only if {@code gwc-gs.xml} didn't already exist.
 * <li>Upgrades the direct WMS integration configuration from an old config.
 * Before using {@code gwc-gs.xml} to hold the integrated GWC configuration, the
 * only property configured was whether the direct WMS integration option was
 * enabled, and it was saved as part of the {@link WMSInfo} metadata map under
 * the {@code GWC_WMS_Integration} key. This method removes that key from
 * WMSInfo if present and sets its value to the {@code GWCConfig} instead.
 * </ul>
 * </p>
 * 
 * @author groldan
...
public class GWCInitializer implements GeoServerInitializer {
	...
	private static final Logger LOGGER = Logging
			.getLogger(GWCInitializer.class);

	/**
	 * {@link WMSInfo#getMetadata() WMSInfo metadata} key used in < 2.1.2 to
	 * hold the gwc direct wms integration enablement flag
	 */
	...
	public GWCInitializer(GWCConfigPersister configPersister,
			Catalog rawCatalog, TileLayerCatalog tileLayerCatalog) {
	...
	public void initialize(final GeoServer geoServer) throws Exception {
		...
		if (currentVersion.compareTo(version) > 0) {
			// got the global config file, so old defaults are already in place
			// if need be. Now
			// check whether we need to migrate the configuration from the
			// Layer/GroupInfo metadata
		...
		if (gwcConfig.getCacheConfigurations() == null) {
			...
			map.put(GuavaCacheProvider.class.toString(),
		...
		if (blobStore != null) {
			...
			if (!blobStore.getCacheProviders().containsKey(cacheProviderClass)) {
				gwcConfig.setCacheProviderClass(GuavaCacheProvider.class
						.toString());
				configPersister.save(gwcConfig);
				if (LOGGER.isLoggable(Level.FINEST)) {
					LOGGER.finest("Unable to find: " + cacheProviderClass
	...
	/**
	 * Migrates the configuration of geoserver tile layers from the
	 * Layer/GroupInfo metadata maps to the {@link #tileLayerCatalog}
	 */
	...
	/**
	 * This method is called only the first time the {@link GWCConfig} is
	 * initialized and is used to maintain backwards compatibility with the old
	 * GWC defaults.
	 */
	private void createDefaultTileLayerInfos(final GWCConfig defaultSettings) {
		...
		for (LayerInfo layer : rawCatalog.getLayers()) {
			...
			try {
				GeoServerTileLayerInfo tileLayerInfo;
				tileLayerInfo = TileLayerInfoUtil.loadOrCreate(layer,
						defaultSettings);
				tileLayerCatalog.save(tileLayerInfo);
				MetadataMap metadata = layer.getMetadata();
				if (metadata
						.containsKey(LegacyTileLayerInfoLoader.CONFIG_KEY_ENABLED)) {
					LegacyTileLayerInfoLoader.clear(metadata);
					rawCatalog.save(layer);
				}
			} catch (RuntimeException e) {
	...
	/**
	 * Before using {@code gwc-gs.xml} to hold the integrated GWC configuration,
	 * the only property configured was whether the direct WMS integration
	 * option was enabled, and it was saved as part of the {@link WMSInfo}
	 * metadata map under the {@code GWC_WMS_Integration} key. This method
	 * removes that key from WMSInfo if present and sets its value to the
	 * {@code gwcConfig} instead.
	 */
	private void upgradeWMSIntegrationConfig(final GeoServer geoServer,
			final GWCConfig gwcConfig) throws IOException {
		// Check whether we're using the old way of storing this information,
		// and get rid of it
		WMSInfo service = geoServer.getService(WMSInfo.class);
		if (service != null) {
			MetadataMap metadata = service.getMetadata();
			if (service != null && metadata != null) {
				Boolean storedValue = metadata.get(WMS_INTEGRATION_ENABLED_KEY,
						Boolean.class);
				if (storedValue != null) {
	...
	/**
	 * Private method for adding all the Layer that must not be cached to the
	 * {@link CacheProvider} instance.
	 * 
	 * @param cache
	 * @param defaultSettings
	 */
	private void addLayersToNotCache(CacheProvider cache,
			GWCConfig defaultSettings) {
		if (LOGGER.isLoggable(Level.FINEST)) {
		...
		for (LayerInfo layer : rawCatalog.getLayers()) {
			...
			try {
				...
				GeoServerTileLayerInfo tileLayerInfo = tileLayerCatalog
						.getLayerById(layer.getId());
				...
				LOGGER.log(Level.WARNING, "Error occurred retrieving Layer '"
						+ layer.getName() + "'", e);
			}
		}
